...
public class FoodTest {
	...
	public void foodCreateTest() {
		...
		Food food = Food.builder().foodName("테스트푸드1").reportNumber("테스트보고번호").category("테스트카테고리")
				.manufacturerName("테스트제조사").foodDetail(foodDetail).foodImage(foodImage).allergyMaterials("알러지 재료테스트")
				.barcodeNumber("바코드 번호0ㅑ09ㅑ090").releaseDate("출시일").price(10000).licenseNumber("라이센스 넘버 테스트").build();
	...
	public void foodReadTest() {
		...
		Optional<Food> findFood = foodRepository.findById(7L); // 아이디 바꿔줘야함
		System.out.println(findFood.get().getFoodName());

		assertThat(findFood.get().getFoodName()).isEqualTo("테스트푸드1");
		...
		assertThat(findFood.get().getReleaseDate()).isEqualTo("출시일");
		assertThat(findFood.get().getLicenseNumber()).isEqualTo("라이센스 넘버 테스트");
		assertThat(findFood.get().getPrice()).isEqualTo(10000);
	...
	public void reviewReadTest() {
		em.flush();
		em.clear();
		Optional<Review> findReview = reviewRepository.findById(5L);
	...
	public void favoriteReadTest() {
		em.flush();
		em.clear();
		Optional<Favorite> findFavorite = favoriteRepository.findById(5L);
		System.out.println("즐겨찾기와 연결된 음식 이름 : " + findFavorite.get().getFood().getFoodName());

		assertThat(findFavorite.get().getFood().getFoodName()).isEqualTo("테스트푸드1");
	...
	public void reviewLikeReadTest() {
		em.flush();
		em.clear();
		Optional<ReviewLike> findReviewLike = reviewLikeRepository.findById(4L);
	...
	public void reviewDeleteTest() {
		...
		Optional<Review> deletedReview = reviewRepository.findById(5L);
	...
	public void favoriteDeleteTest() {
		...
		Optional<Favorite> deletedFavorite = favoriteRepository.findById(5L);
	...
	public void reviewLikeDeleteTest() {
		...
		Optional<ReviewLike> deletedReviewLike = reviewLikeRepository.findById(4L);
